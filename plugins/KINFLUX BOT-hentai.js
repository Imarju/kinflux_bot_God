let handler = async (m, { conn, text, usedPrefix, command, args }) => {
    if (!global.db.data.chats[m.chat].nsfw) throw `🚫 group doesnt supprt gmx bot nsfw \n\n enable it by \n*${usedPrefix}enable* nsfw`
    let user = global.db.data.users[m.sender].age
    if (user < 17) throw m.reply(`🌚 u need to be atleast 18 years old`)
    if (!text) throw `*This command provides sauce from nhentai: ${usedPrefix + command} waifu*`
    try {
    m.reply(global.wait)
    ...
    throw `*SORRY BRUHH ⚠️ GMX INTERNAL ERROR NOT FOUND TRY SEARCHING ANOTHER QUERY*`
    }}
