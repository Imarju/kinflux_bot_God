...
let quranSurahHandler = async (m, { conn }) => {
  try {
    ...
    if (!surahInput) {
      throw new Error(`لطفا شماره یا نام سوره را مشخص کنید، زبان ارائه شده در ربات gmx توسط Arju-sonwani`);
    }
    ...
    if (!surahData) {
      throw new Error(`سوره با شماره یا نام پیدا نشد "${surahInput}"`);
    }
